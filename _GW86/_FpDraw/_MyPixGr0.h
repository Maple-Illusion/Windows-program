//GW86_FpDraw_MyVGra0.h.
//Gao-Wei Chang, 220507, 201012, 200606, 180611, 160707, 140307.
#pragma once
#include "C:\Users\ataraxia\Documents\AV_LAB\NTNU_Cpp\_GW86\_FpDraw\_MyPixRect0.h"
#include "C:\Users\ataraxia\Documents\AV_LAB\NTNU_Cpp\_GW86\_ClrMath\_MySExppStat0.h"
#define PI 3.14159
namespace GW86 {
namespace FpDraw {
  ...
  using namespace GW86::FpDraw;//新增
...
public:
static void drawSinSegs(Graphics^ g1, MyRect0^ theCoord0, Pen^ myPen1,
	double A, double T, double val2phase, double offset, array<float>^ xyspan, int nPt) {
...
static void drawSine(Graphics^ g1, MyRect0^ theCoord0, Pen^ myPen1,
  ...
  drawSinSegs(g1, theCoord0, myPen1, A, T, val2phase, offset, xyspan, nPt);
};

public:
static void drawSinw(Graphics^ g1, MyRect0^ theCoord0, Pen^ myPen1,
  array<double>^ opds, array<float>^ xyspan, int nPt, double xDisp) {
	double A, T, val2phase, offset; //cyc = 2;
	A = opds[0]; T = opds[1]; //週期T = xmax/cyc;
	val2phase = opds[2]-xDisp; offset = opds[3];
	drawSinSegs(g1, theCoord0, myPen1, A, T, val2phase, offset, xyspan, nPt);
};
