//GW86_ClrMath_CmTaskSEs0.h
//Gao-Wei Chang, 180424, 170610, 160701.
#pragma once
#include "c:\_GW86_ClrMath\tasks\_seriesExpp0x.h"
namespace GW86 {
...
public:
  static void seriesExpp(int opcode_id, double x, int n) {
	...
	switch (opcode_id) {
	  ...
	  case ID_SINC:
		 for (int i = 0; i<n; i++)
		   Console::WriteLine("For {0}-th iteration, sinc(2*PI*{1:F3})= {2}.",
				i, ratio_x, obj.sinc(theta_x, i));
		break;
	  case ID_ATAN:
		 for (int i = 0; i<n; i++)
		   Console::WriteLine("For {0}-th iteration, atan({1:F3})= {2}.",
				i, ratio_x, obj.atan(x, i));
		break;
	}; 
  };
};
