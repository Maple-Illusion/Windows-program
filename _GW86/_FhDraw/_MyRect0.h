//GW86_FhDraw_MyRect0.h
//Gao-Wei Chang, 210826, 210815, 200606, 180526, 160614, 140711, 131228.
#pragma once
namespace GW86 {
 namespace FhDraw {
...
public:
  ...
  void drawRectCoord(Graphics^ g1, float xmin, float xmax,
	...
	double xval, yval; float xloc, yloc;

	for (int i=0; i<=nDiv; i++) {
	  xval=xmin+(i/nDiv)*(xmax-xmin);
	  yval=ymin+(i/nDiv)*(ymax-ymin);
	  if ((xval < 0.01) && (xval > -0.01)) xval = 0;
	  if ((yval < 0.01) && (yval > -0.01)) yval = 0;
	  //增訂210805、210815日，用以避免過長小數
	  ...
	  g1->DrawString(xval.ToString("F1"), cnFont9, //標示x(y)讀值變數
			  Brushes::Gray, xloc+xval_xdelta, y_end+xval_ydelta);
	  g1->DrawString(yval.ToString("F1"), cnFont9,
...
public:
  void drawRectCoord(Graphics^ g1, array<float>^ xyspan, float nDiv) {
	  float xmin = xyspan[0]; float xmax = xyspan[1];
	  float ymin = xyspan[2]; float ymax = xyspan[3];
	  drawRectCoord(g1, xmin, xmax, ymin, ymax, nDiv);
  };

public:
  void drawRectCoord(Graphics^ g1, array<float>^ xyspan, float xDiv, float yDiv) {
    float xmin = xyspan[0]; float xmax = xyspan[1];
	float ymin = xyspan[2]; float ymax = xyspan[3];
	float x_div = float((x_end - x_begin) / xDiv);
	float y_div = float((y_end - y_begin) / yDiv);
	double xval, yval; float xloc, yloc;

     for (int i = 0; i <= xDiv; i++) {
		xval = xmin + (i / xDiv) * (xmax - xmin);
		if ((xval < 0.01) && (xval > -0.01)) xval = 0;
		xloc = x_begin + (i * x_div);
		if (xval == 0) g1->DrawLine(lightGrayPen2, xloc, y_begin, xloc, y_end);
		g1->DrawString(xval.ToString("F1"), cnFont9, //標示x(y)讀值變數
				Brushes::Gray, xloc + xval_xdelta, y_end + xval_ydelta);
		if (i == 0 || i == xDiv) {//繪製座標邊框
			g1->DrawLine(blackPen1, xloc, y_begin, xloc, y_end); }
		else {//繪製格線
			g1->DrawLine(lightGrayPen1, xloc, y_begin, xloc, y_end);
		};
	...
	for (int i = 0; i <= yDiv; i++) {
	  yval = ymin + (i / yDiv) * (ymax - ymin);
	  if ((yval < 0.01) && (yval > -0.01)) yval = 0;
	  yloc = y_end - (i * y_div);
	  if (yval == 0) g1->DrawLine(lightGrayPen2, x_begin, yloc, x_end, yloc);
	  g1->DrawString(yval.ToString("F1"), cnFont9,
				Brushes::Gray, x_begin + yval_xdelta, yloc + yval_ydelta);
	  if (i == 0 || i == yDiv) {//繪製座標邊框
		g1->DrawLine(blackPen1, x_begin, yloc, x_end, yloc); }
	  else {//繪製格線
		g1->DrawLine(lightGrayPen1, x_begin, yloc, x_end, yloc);
	  };
